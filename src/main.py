...
_TOOL_SECIMI = (
    ...
    "sonucu kendin okunaklı bir markdown'a çevir; kullanıcı açıkça ham/JSON çıktı istemedikçe "
    "JSON'u kullanıcıya gösterme. Kullanıcı 'json olarak ver', 'ham veri', 'raw' gibi net bir "
    "istekte bulunursa tool'un döndürdüğü JSON'u ```json kod bloğu içinde OLDUĞU GİBİ ver. "
    "Biçimlendirme için SADECE düz markdown kullan (madde işareti, kalın, başlık); "
    "LaTeX matematik sözdizimi ($...$, \\text{}, \\rightarrow gibi) KULLANMA — Telegram "
    "bunu render etmez, kullanıcıya ham kod olarak görünür. 'artış', 'oran', '->' gibi "
    "ifadeleri düz Türkçe kelime veya normal karakterle yaz (ör. '10 kat', 'destek', '→' "
    "yerine 'ile sonuçlanır'). "
